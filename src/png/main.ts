...
import { PNG_SIGNATURE, CHUNK_TYPE, COLOR_TYPE, INTERLACING, Interlacing } from './constants.js';
import { BitDepth, Channels, FilterTypes } from './types.js';
...
export class PngDecoder {
	...
	private scaleBitDepth(buffer: Buffer, width: number, heigth: number): Buffer {
		...
		case 1:
			...
			if (this.colorType !== COLOR_TYPE.IndexedColor) {
				for (let i = 0; i < chunk.length; i += 1) {
					chunk[i] *= 0xff;
				}
			}
	...
	private convertToRGBA(buffer: Buffer, width: number, heigth: number): Buffer {
		...
		case COLOR_TYPE.Grayscale:
			for (let i = 0, k = 0; i < buffer.length; i += 1, k += 4) {
				if (this.transparent?.[0] === buffer[i]) {
					continue;
				}
